...
function CreateSlip(props) {
  ...
  const { data, setData, post, get, processing, errors, reset } = useForm({
    users: props.slip.map((slip) => ({
      ...
      mk: slip.mk,
      absen: slip.absen,
      total: (parseFloat(slip.gaji_pokok) || 0) + (parseFloat(slip.gaji_lembur) || 0) + (parseFloat(slip.tj_jabatan) || 0) + (parseFloat(slip.tj_kehadiran) || 0) + (parseFloat(slip.tj_kinerja) || 0) - (parseFloat(slip.bpjs) || 0) - (parseFloat(slip.pinjaman) || 0) - (parseFloat(slip.lain_lain) || 0)
    }))
  });
